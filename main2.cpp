...
public:
    ...
    int minimax(vector<vector<string>> state, bool aiMove, int depth) // DODANO depth
    {
        ...
        {
            if(score == 1)
                return 10 - depth; // DODANO: Odejmowanie depth, aby faworyzować szybsze wygrane (przyklad 10 - 1 = 9 > 10 - 3 = 7(pominie slabsza))
            else if(score == -1)
                return -10 + depth; // DODANO: Dodawanie depth, aby opóźniać przegrane (analogicznie)
            else return 0;
        }
        
        // DODANO: Sprawdzenie, czy osiągnięto maksymalną głębokość
        if (depth >= maxDepth) {
            return 0; // DODANO: Heurystyczna ocena na limicie głębokości
        }
    ...
    // alpha-beta
    int AB_minimax(vector<vector<string>> state, bool aiMove, int alpha, int beta, int depth) // DODANO: depth
    {
        ABcounter++;
        int score = isEndState(state);
        if (score != 2)
        ...
        if(aiMove)
        {
            ...
            for(int i = 0; i < boardSize; i++)
            {
                for(int j = 0; j < boardSize; j++)
                {
                    ...
                    {
                        state[i][j] = "X";
                        int val = AB_minimax(state, false, alpha, beta, depth + 1); // DODANO:  depth + 1
                        state[i][j] = " ";
                        best = max(best, val);
    ...
    pair<int, int> bestMove() // ZMIANA W SRODKU (aby uwzglednic depth)
    {
        ...
        for(int i = 0; i < boardSize; i++)
        {
            for(int j = 0; j < boardSize; j++)
            {
                ...
                {
                    board[i][j] = "X";
                    int val = minimax(board, false, 0); // DODANO: depth = 0
                    board[i][j] = " ";
    ...
    pair<int, int> AB_bestmove() // ZMIANA W SRODKU
    {
        int bestVal = numeric_limits<int>::min();
        pair<int,int> move = {-1, -1};
        ...
        for(int i = 0; i < boardSize; i++)
        {
            for(int j = 0; j < boardSize; j++)
            {
                ...
                {
                    board[i][j] = "X";
                    int val = AB_minimax(board, false, numeric_limits<int>::min(), numeric_limits<int>::max(), 0); // DODANO: depth = 0
                    board[i][j] = " ";
...
void selectDifficulty() {
    int choiced;
    cout << "Wybierz poziom trudności AI:\n";
    cout << "1. Łatwy (głębokość 4)\n2. Średni (głębokość 8)\n3. Trudny (głębokość 16)\n4. Niemożliwy (głębokość 32)\n";
    cout << "Wprowadź wybór (1-4): ";
    cin >> choiced;

    switch (choiced) {
        case 1:
            maxDepth = 4;
            cout << "Poziom trudności ustawiony na Łatwy (głębokość = 4)\n \n";
            break;
        case 2:
            maxDepth = 8;
            cout << "Poziom trudności ustawiony na Średni (głębokość = 8)\n \n";
            break;
        case 3:
            maxDepth = 16;
            cout << "Poziom trudności ustawiony na Trudny (głębokość = 16)\n \n";
            break;
        case 4:
            maxDepth = 32;
            cout << "Poziom trudności ustawiony na Niemożliwy (głębokość = 32)\n \n";
            break;
        default:
            maxDepth = 8;
            cout << "Nieprawidłowy wybór, ustawiono Średni (głębokość = 8)\n \n";
    }
}
int main()
{
